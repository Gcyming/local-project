...
> 早期（08-10 之前）核心在 `D:\tool\slime`（Python 单栈），08-17 起冷迁移至 `D:\pilot project`（TS 双栈）。引擎源码、GUI、测试与文档**全部随仓库公开**（2026-09-17 起）；仅密钥、隐私数据与可再生产的产物不入库。
...
2. **仓库全量公开**：引擎、GUI、测试、文档一并入库，供他人阅读与审阅。
   `.gitignore` 只排除三类 —— **密钥与隐私**（`config/` 里的 API Key 密文、`data/` 里的对话与用量）、
   可再生产的产物（依赖 / 构建输出 / 缓存）、以及体积资产（模型权重 / 虚拟环境）。**不含任何"藏代码"策略。**
...
4. **~~引擎源码零版本控制~~（已于 2026-09-17 解决）**：该条记录的是历史状态 —— 早期为控制仓库体积，
   引擎目录曾被排除在版本控制之外，导致 `core-ts` 长期只有单副本裸盘、无历史无备份。
   现在引擎、GUI、测试、文档已全部入库，风险消除；仅密钥与隐私数据不入库。
