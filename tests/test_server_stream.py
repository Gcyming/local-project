...
class TestChatStreamEndpoint:
    ...
    def test_stream_delegation_emits_heartbeat(self):
        ...
        async def fake_stream(a, msg, history=None, providers=None,
                              registry=None, system_prompt=None, **kw):
            call_count[0] += 1
            if call_count[0] == 1:
    ...
    def test_chat_endpoint_no_name_error(self):
        ...
        A-120：无 provider 时 as_brain 开启 → SILAM 大脑兜底（model=silam-brain）；
        关闭 → 原默认提示（model="none"，向后兼容）。开关显式 patch，不依赖 slime.toml 状态。"""
        slime_server, agent, client = self._env()
        try:
            with self._noop_patches(slime_server), \
                    patch("core.llm._silam_as_brain", return_value=True):
                r = client.post(
...
class TestSkillEvidenceInjection:
    ...
    def test_skill_hit(self):
        ...
        candidates = sorted(p.name for p in root.iterdir() if p.is_dir()) if root.is_dir() else []
        slug = next((c for c in candidates if any(h.get("name") == c for h in reg.search(c))), None)
        assert slug, "技能库中找不到可精确命中的技能（检查 config/skills 是否为空）"
        r = S._inject_skill_evidence(f"使用技能 {slug} 处理：ultra")
        assert f"[平台证据] 技能 {slug} 已确认存在" in r
