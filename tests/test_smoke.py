...
class TestEncryption:
    ...
    ⚠️ 两个用例**都必须在临时目录里跑**，不能写进项目的真实 `config/`。
    踩过的坑：`encrypt()` 会调 `_harden_file()` 对落盘文件做 Windows 权限加固
    （`icacls /inheritance:r /grant:r <user>:(M)` + 隐藏属性）。一旦清理环节没跑成
    （例如被沙箱/杀软拦下 `unlink`），`config/test_*.enc.json` 就以**加固后的权限**留在盘上；
    下一次运行时 `encrypt()` 覆写该文件直接 `PermissionError: [Errno 13]` ——
    一次性红掉 run_tests 与 pytest 两个阶段，且报错点离真正原因很远。
    用 `TemporaryDirectory` 之后，加固只作用在临时文件上，与仓库状态彻底解耦。
    """
    ...
    def test_encrypt_decrypt_roundtrip(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            test_path = Path(td) / "test_roundtrip.enc.json"
    ...
    def test_encrypt_empty_dict(self):
        ...
        from core.encryption import encrypt, decrypt
        with tempfile.TemporaryDirectory() as td:
            test_path = Path(td) / "test_empty.enc.json"
