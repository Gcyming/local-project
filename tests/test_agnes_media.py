...
class TestMediaCache:
    ...
    def test_prune_cache_ttl(self, tmp_path):
        """A-970：过期文件（超 TTL）被清理，新鲜文件保留"""
        import os
        import time
        from tools import agnes_media as M
        d = tmp_path / "images"
        d.mkdir()
        fresh = d / "cache_fresh.png"
        fresh.write_bytes(b"x" * 10)
        stale = d / "cache_stale.png"
        stale.write_bytes(b"y" * 10)
        old = time.time() - M._MEDIA_CACHE_TTL_SECONDS - 60
        os.utime(stale, (old, old))
        M._prune_media_cache_dir(d)
        assert not stale.exists()
        assert fresh.exists()

    def test_prune_cache_capacity(self):
        """A-970：容量超上限 → 按 mtime 从旧到新淘汰

        ⚠️ 本方法**故意不用 pytest 夹具参数**（`tmp_path` / `monkeypatch`）。
        本仓库有两套测试运行器：pytest，以及自研的 `run_tests.py`（反射直调 `test_` 方法、
        不注入任何夹具）。带夹具参数的用例在后者下必然
        `TypeError: missing 1 required positional argument` —— 这条用例就是这样长期红着的，
        而它红的原因与业务代码毫无关系，纯属"测试与运行器的契约不匹配"。
        改用 stdlib 的 TemporaryDirectory + 手动保存/恢复模块属性，两套运行器都能跑。
        """
        import os
        import time
        import tempfile
        from pathlib import Path
        from tools import agnes_media as M
        saved_max = M._MEDIA_CACHE_MAX_BYTES
        try:
            ...
            M._MEDIA_CACHE_MAX_BYTES = saved_max
