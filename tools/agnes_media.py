...
# A-970：磁盘缓存加界（问题5）——内容寻址缓存 cache_*.{ext} 与下载产物 {ts}_{md5}.{ext}
# 同目录，此前无 TTL / 容量上限会无限膨胀。图片/视频按 kind 目录分别加界。
_MEDIA_CACHE_TTL_SECONDS = 7 * 24 * 3600      # 7 天过期
_MEDIA_CACHE_MAX_BYTES = 512 * 1024 * 1024    # 512 MB（per kind 目录）
                ...
                with open(out_path, "wb") as f:
                    async for chunk in resp.aiter_bytes():
                        ...
                        if total > _MAX_DOWNLOAD_BYTES:
                            ...
                            return ""
                        f.write(chunk)
        _prune_media_cache_dir(out_dir)  # A-970: 写后加界（TTL + 容量）
        return str(out_path)
...
def _prune_media_cache_dir(d: Path) -> None:
    """A-970：对 data/generated/{images|videos} 目录做加界清理。
    ① 删除 mtime 超过 TTL 的过期文件；② 仍超容量上限时按 mtime 从旧到新淘汰。
    尽力而为（失败仅 warning），绝不阻塞生成主流程。"""
    try:
        now = time.time()
        for p in [x for x in d.iterdir() if x.is_file()]:
            try:
                if now - p.stat().st_mtime > _MEDIA_CACHE_TTL_SECONDS:
        ...
        while total > _MEDIA_CACHE_MAX_BYTES and remaining:
        ...
        logging.warning(f"[agnes_media] 缓存加界清理失败: {e}")
