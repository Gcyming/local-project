...
export function buildDelegationPrompt(children: Array<{ name: string; role?: string }>, allAgents?: string[]): string {
  ...
  if (allAgents && allAgents.length > 1) {
    lines.push("### 委派给任意已注册 Agent");
    lines.push("你可以把子任务委派给任何已在平台的 Agent（不限于子 Agent）：");
    lines.push("`<DELEGATE name=\"Agent 名\">具体子任务</DELEGATE>`");
    lines.push(`当前可委派的 Agent：${allAgents.join(", ")}`);
    lines.push("");
    lines.push("### 广播");
