...
export class ModelBackend {
  ...
  start(args: BackendArgs): boolean {
    ...
    if (args.embedding) {
      ...
      // 聊天模型：按模型名选 reasoning 格式，把思考块提取到 message.reasoning_content（协议层分离思考与正文）。
      // - Qwen3 系（文件名含 qwen）：原生模板输出 `<thinking>…</thinking>` 标签 → 用 `qwen` 格式解析；
      // - 其余（DeepSeek R1 / 蒸馏模型）：` thinking…response` 无尖括号块 → 用 `deepseek` 格式解析。
      // 非思考模型无思考标签则原样返回，不受影响。前端 createThinkingStripper 仍作边到边剥离兜底。
      const modelName = basename(args.modelPath).toLowerCase();
      argv.push("--reasoning-format", modelName.includes("qwen") ? "qwen" : "deepseek");
    }
  ...
  async waitReady(timeout = 120, signal?: AbortSignal): Promise<boolean> {
    ...
    while (Date.now() < deadline) {
      ...
      if (this.process && this.process.exitCode !== null && this.process.exitCode !== undefined) {
        console.warn(`[model_server] llama-server 进程已退出（exit ${this.process.exitCode}），加载失败`);
        return false;
      }
      try {
...
export class ModelServerManager {
  ...
  async ensure(role: string, modelPath = "", modelName = "", opts: EnsureModelOpts = {}): Promise<EnsureResult> {
    ...
    const fast = await this.reuseIfReady(role, modelPath);
    if (fast) return fast;

    // 2. 关键段加锁（H2：防止并发双启动）
    return this.ensureLock.run(async () => {
      // 2a. 双检：锁内再查一次
      const locked = await this.reuseIfReady(role, modelPath);
      if (locked) return locked;
      return this.ensureLocked(role, modelPath, modelName, cfg, opts);
    });
  }

  private async reuseIfReady(role: string, matchModel = ""): Promise<EnsureResult | null> {
  ...
  private async ensureLocked(
    ...
    if (role === "chat" && modelPath) {
      ...
      if (prev && !prev.external && prev.model_path && prev.model_path !== modelPath) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      const inst: Instance = {
        ...
        ctx_len: (opts.ctxLen ?? (cfg.ctx_len as number | undefined)) ?? 2048,
        external: false,
      };
      ...
      inst.error = backend.hasExited()
        ? "llama-server 进程已退出（模型文件可能损坏或不兼容）。请确认 .gguf 文件完整未损坏，或重新下载。"
        : `启动超时（${this.startupTimeout}s 内未就绪）。若为新下载的模型，请确认 .gguf 文件完整未损坏。`;
      this.writeRegistry();
  ...
  status(): StatusItem[] {
    ...
    if (!this.instances["embedding"] && this.embedCfg.persistent) {
      ...
      items.push({
        ...
        port: (this.embedCfg.port as number) ?? 8999,
        pid: null,
  ...
  /** chat 实例是否已就绪且模型匹配（决定是否需要弹「加载本地模型」面板；模型切换/未启动 → false） */
  isChatReady(modelPath = ""): boolean {
    const inst = this.instances["chat"];
    if (!inst || inst.state !== ServerState.READY) return false;
    if (modelPath && inst.model_path && inst.model_path !== modelPath) return false;
    return true;
  }
