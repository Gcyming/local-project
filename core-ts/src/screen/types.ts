...
export interface ScreenBackend {
  ...
  /**
   * A-1069（可选，桌面后端）：报告**常驻辅助进程**的存活状态 —— 供「Agent 启动的后台进程」
   * 面板展示与终止（#226）。
   *
   * 存在的理由：宿主进程是**懒启动**的（首次 `screen_*` 时 spawn），外部无从知道它何时起落。
   * 让后端自报，就不用在外层维护一份必然与真实漂移的镜像状态。
   * 返回 `null` = 当前没有常驻进程。不实现它的后端（如 adb 驱动的 Android）自然不产生条目。
   */
  residentHost?(): { pid?: number; startedAt: number } | null;
  /**
   * A-1069（可选）：释放该后端持有的**常驻进程/资源**。
   *
   * ⚠️ 与 `residentHost()` 成对出现：能报告"我持有一个后台进程"的后端，就必须能释放它 ——
   *   否则面板上会出现一个**按了没反应**的停止按钮（比不显示更糟）。
   */
  dispose?(): void;
}
