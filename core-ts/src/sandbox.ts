...
export class SandboxManager {
  ...
  constructor(config?: SandboxConfig, approvalCallback?: (req: PermissionRequest) => ApprovalDecision | Promise<ApprovalDecision>) {
    this.config = config ?? defaultSandboxConfig();
    this.approvalCallback = approvalCallback ?? null;
  }

  setApprovalCallback(cb: (req: PermissionRequest) => ApprovalDecision | Promise<ApprovalDecision>): void {
  ...
  clearSessionAllowlist(agentId = ""): void {
    if (agentId) {
      ...
      this.sessionToolAllowlist.clear();
    }
  }
    ...
    if (target && (cfg.allowPaths ?? []).length > 0) {
      for (const ap of cfg.allowPaths ?? []) {
        if (ap && validateWorkspace(ap, target)) {
          return { allowed: true, reason: "自定义白名单（权限设置放行目录）", level, anomalyDetected: false, anomalyAlerts: [] };
        }
      }
    }
  ...
  async grantPermission(opts: {
    ...
    taskId?: string;
    sessionId?: string;
  }): Promise<{ allowed: boolean; reason: string; anomalyDetected: boolean; anomalyAlerts: string[] }> {
    return this.grantCore(opts);
  }

  private async grantCore(opts: {
    agentId: string;
    action: string;
    target: string;
    level: number;
    ...
    if (target && (cfg.allowPaths ?? []).length > 0) {
      for (const ap of cfg.allowPaths ?? []) {
        if (ap && validateWorkspace(ap, target)) {
          ...
          this.writeAudit(makeAuditEntry({ agent_id: agentId, task_id: taskId, action, target, level, status: "allowed", granted_by: "user", grant_id: gid, details: { reason: "自定义白名单（预设放行目录）" } }));
          return { allowed: true, reason: "自定义白名单（权限设置放行目录）", anomalyDetected: false, anomalyAlerts: [] };
        }
      }
    }
    ...
    if (cfg.workspace && target && !validateWorkspace(cfg.workspace, target)) {
      if (cfg.allowOutsideWorkspace) {
        ...
        this.writeAudit(makeAuditEntry({ agent_id: agentId, task_id: taskId, action, target, level, status: "allowed", granted_by: grantedBy, grant_id: gid, details: { reason: "审批档位：工作目录外自动放行" } }));
        return { allowed: true, reason: "审批档位自动放行（工作目录外）", anomalyDetected: false, anomalyAlerts: [] };
      }
      if (this.sessionToolAllowed(agentId, action)) {
        const gid = this.recordGrant(agentId, action, target, level, "user");
        this.writeAudit(makeAuditEntry({ agent_id: agentId, task_id: taskId, action, target, level, status: "allowed", granted_by: "user", grant_id: gid, details: { reason: "会话级总是允许（工作目录外）" } }));
        return { allowed: true, reason: "会话级已批准", anomalyDetected: false, anomalyAlerts: [] };
      }
      const r = await this.approvalPath({ agentId, action, target, level, taskId, anomalyDetected: false, rule: "工作目录外", sessionId: opts.sessionId });
      return { ...r, anomalyDetected: false, anomalyAlerts: [] };
    }
    ...
    if (cfg.deny_levels.includes(level) || toolMatches(action, cfg.deny_tools)) {
      ...
      if (cfg.allowDeny) {
        ...
        return { allowed: true, reason: "无需审批：已放行", anomalyDetected, anomalyAlerts };
      }
      if (cfg.askOnDeny) {
        const r = await this.approvalPath({ agentId, action, target, level, taskId, anomalyDetected, rule: "系统拒绝", sessionId: opts.sessionId });
    ...
    if (anomalyDetected) {
      for (const rule of this.anomalyDetector.rules) {
        if ((ruleMatches(rule, action, target) || anomalyAlerts.includes(rule.description)) && (rule.action === "deny" || rule.action === "deny+alert" || rule.action === "terminate")) {
          ...
          if (cfg.allowDeny) {
            const gid = this.recordGrant(agentId, action, target, level, grantedBy);
            this.writeAudit(makeAuditEntry({ agent_id: agentId, task_id: taskId, action, target, level, status: "allowed", granted_by: grantedBy, grant_id: gid, anomaly_detected: true, details: { rule: rule.name, reason: "无需审批档：异常风险已放行" } }));
            return { allowed: true, reason: "无需审批：异常风险已放行", anomalyDetected: true, anomalyAlerts };
          }
          if (cfg.askOnDeny) {
            const r = await this.approvalPath({ agentId, action, target, level, taskId, anomalyDetected, rule: "异常风险", sessionId: opts.sessionId });
            return { ...r, anomalyDetected, anomalyAlerts };
          }
          this.writeAudit(makeAuditEntry({ agent_id: agentId, task_id: taskId, action, target, level, status: "denied", granted_by: grantedBy, risk_score: 1, anomaly_detected: true, details: { rule: rule.name, action: rule.action } }));
          return { allowed: false, reason: `异常操作被拒绝: ${rule.description}`, anomalyDetected: true, anomalyAlerts: [...anomalyAlerts, rule.description] };
    ...
    if (cfg.require_approval_levels.includes(level)) {
      const r = await this.approvalPath({ agentId, action, target, level, taskId, anomalyDetected, rule: "等级", sessionId: opts.sessionId });
      return { ...r, anomalyDetected, anomalyAlerts };
    }
  ...
  private async approvalPath(opts: {
    agentId: string; action: string; target: string; level: number; taskId: string; anomalyDetected: boolean; rule: string; sessionId?: string;
    ...
    if (decision.approved) {
      ...
      return { allowed: true, reason: decision.reason || "用户批准" };
    }
    ...
    return { allowed: false, reason: decision.reason ? `用户拒绝: ${decision.reason}` : "用户拒绝" };
  }
    ...
    try {
      ...
      const keptLines = raw.split("\n").filter((l) => {
        ...
        try {
          ...
          return typeof e.timestamp === "string" && e.timestamp >= cutoff;
        } catch {
          return false;
        }
      });
