...
function inferModelDefaults(modelId: string): Partial<ModelSpec> {
  ...
  if (context_window === undefined) {
    ...
    /* Agnes：官方口径「512K」。
     *
     * ⚠️ A-1087：这里原先写的理由是「显示层统一 ÷1000，写 524288 就会读成 524K」——
     * **那条理由已作废**。显示层现在是**按上限自适进制**（`contextMath.pickTokenBase`：
     * 哪个进制能把上限写成整数 K 就用哪个，平局取十进制），`524288` 会正确读作「512K」、
     * `512000` 也读作「512K」，两者在界面上**完全同形**。别再把「÷1000」当成全局口径去
     * 反推数值——右栏监测 / 供应商编辑器 / 新建会话选模型三处都已收口到同一个函数。
     *
     * 保留 `512000`（而不是改回 2^19）的理由只剩下"不改数值、少一次 churn"：本分支是
     * **能力表未命中时的兜底推断值**（不是探测到的真值），而 512K 在两种进制下都成立，
     * 且以十进制写法与同表其余头条模型（gpt-4o 128000 / claude 200000 / deepseek 64000）同形。
     * 代价是比 2^19 少报 2.3%（12288 token），对一个兜底推断值而言无关紧要。
     */
