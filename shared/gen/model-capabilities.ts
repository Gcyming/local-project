...
export const MODEL_CAPABILITIES: VendorCapabilities[] = [
  ...
  },
  {
    ...
    // ── 上下文口径（A-1054 修正，**这是"配置 512K、界面 524K"的真根因**）──
    // 官方文档「限制与价格」表：上下文窗口 **512K** / 最大输出 **65.5K**
    //   （agnes-ai.com/zh-Hans/docs/agnes-25-flash）。
    //
    // ⚠️ 表里**必须**写十进制 `512000`，不能写 `524288`（= 512×1024）。理由不是"哪个更接近真值"，
    //    而是**全项目的显示口径是十进制 K（÷1000）**：`ProvidersPanel` 的「上下文K」列把
    //    `context_window / 1000` 直接渲染进输入框，右栏进度条与 `fmtK` 同一口径。
    //    写 524288 就会在界面上读成 **"524K"**，与官方 512K、与用户设置、与媒体模型的显示
    //    三处全对不上 —— 这正是用户报的"内置表出问题了"。
    //
    // ⚠️ 另一条路径也早就按 512000 写了（`providers.ts` 旧正则兜底分支，注释明写"不要改回 524288"），
    ...
    //    Agnes 官方只给"512K"这个十进制写法，与我们表内其余头条模型
    //    （gpt-4o 128000 / claude 200000 / deepseek 64000）同形。**判据是"厂商怎么定义 K"，
    //    不是"我们偏好哪种数制"。**
    //
  ...
  },
  {
    ...
    // A-1054：`524288` → `512000`，与 agnes 同一条口径修正（显示层 ÷1000 → 界面读作 512K）。
    // 这条此前写成 2^19，界面显示 "524K"，与注释自己写的"官方公布窗口 512K"**互相矛盾**。
    context: 512000,
    maxOut: 65536,
    models: [
