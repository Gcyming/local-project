...
async def lifespan(app: FastAPI):
    ...
    # A-126 P0: 注册 Git 治理工具（身份铁律 + 质量门禁 + 语义化提交）
...
class AgentCreate(BaseModel):
    ...
    # 创建默认 = 本地自研 SILAM（双脑）；不再默认 inherit"无模型"，省去绕道供应商
    model_choice: str = Field(default="silam", max_length=100)
...
class ChatRequest(BaseModel):
    ...
    # 识图（A-识图）：data URL 列表（data:image/png;base64,...）。服务端做形态过滤，
    # 深层校验（大小/数量上限）由 core.llm._sanitize_image_data_url 统一兜底。
    images: list[str] = Field(default_factory=list,
                              description="识图图片（data URL）")

    @field_validator("images")
    @classmethod
    def _validate_images(cls, v: list) -> list:
        # 只拒绝明显非法的条目（非字符串/非 data:image 前缀），并裁到 8 张上限
        out = []
        for item in v:
...
def _message_for_history(req: ChatRequest) -> str:
    """写历史/记忆用文本留痕：原文 + 附带图片说明（不持久化 base64，防 history.jsonl 膨胀）。"""
    if not getattr(req, "images", None):
        return req.message
    n = len([i for i in req.images if isinstance(i, str) and i.startswith("data:image/")])
    if n == 0:
        return req.message
...
async def chat(agent_id: str, req: ChatRequest):
    ...
    result = await call_llm_with_meta(agent, effective_message, req.history, providers, agents,
                                      system_prompt=custom_sys, images=req.images)
    ...
    hist_msg = _message_for_history(req)
    agent.persona.add_interaction(hist_msg, raw_reply, success=success)
    history_append(agent.id, hist_msg, raw_reply, success=success)
...
async def _post_process_chat(agent, req, reply: str, success: bool, providers: dict):
    ...
    try:
        # 带图请求：记忆/知识/情绪信号统一用文本留痕（不解析 base64 进记忆）
        hist_msg = _message_for_history(req)
        # ── 记忆提取 ──
        memory_cfg = _SLIME_CONFIG.get("memory", {})
        trait_signals = []
        ...
        async with _get_evolve_lock():
            ...
            agent.emotion.update(
                ...
                failure_type=None,
                novelty=_detect_novelty(agent.id, hist_msg),
                violation=violation,
                praise=_detect_praise(hist_msg, user_sentiment),
            )
        ...
        try:
            ...
            if done_received or full_reply or error_msg:
                ...
                if req.retry:
                    history_pop_last(agent.id)
                hist_msg = _message_for_history(req)
                agent.persona.add_interaction(hist_msg, reply, success=success)
                history_append(agent.id, hist_msg, reply, success=success)
