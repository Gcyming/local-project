...
def load_agents() -> list[Agent]:
    ...
    A-989：`read_json_safe` 在**主文件损坏/半截**时自动回退到 `.bak`（上一份完好内容），
    并把坏文件改名 `.corrupt` 留证。此前直接 `read_text` → JSON 解析失败就返回空列表，
    用户会看到"我所有 Agent（含人格与模型选择）一夜之间全没了"——而现场其实还在盘上。
    """
    from core.safe_io import read_json_safe
    data = read_json_safe(AGENTS_PATH, default=None)
    if data is None:
        return []
    if not isinstance(data, list):
        print("[slime] WARNING: agents.json 顶层不是列表，按空列表处理。", file=sys.stderr)
        return []
    try:
        agents = []
        for a in data:
...
def save_agents(agents: list[Agent]):
    """保存所有 Agent 到 config/agents.json（崩溃安全写：tmp + fsync + replace + .bak）

    A-989：改用 `core.safe_io.atomic_write_text`。此前只做了 tmp+replace，**漏了 fsync** ——
    而 rename 是元数据操作，可能先于文件数据落盘；断电/强杀后会出现"agents.json 在、
    内容是空的或半截"，重启时读它直接炸，用户所有 Agent（含人格/模型选择）看起来全丢。
    """
    ...
    with _save_agents_lock:
        ...
        atomic_write_text(AGENTS_PATH, data)
...
class SILAMBehaviorProxy:
    """兼容 BehaviorStore 接口的代理：内部委托真实行为库，保留 dendrites 句柄。"""
...
class ExplorerProxy:
    ...
    def observe(self, user_input: str, response: str, success: bool):
        """观察一次交互，触发潜在的学习"""
        self._explorer.observe_interaction(user_input, response, success)


def create_explorer(agent):
    """为 agent 创建探索器（始终创建，无论是否使用 SILAM 模型）"""
    try:
    ...
    async def search_callback(query: str) -> str:
        ...
        try:
            ...
            results = await asyncio.run(engine.search(query, max_results=5))
            return results
        except Exception as e:
            import logging
            logging.getLogger("slime.agent").error(f"搜索失败: {e}")
            return f"[搜索失败: {query}]"
