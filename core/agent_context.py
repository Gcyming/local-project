...
# ── A-126 Git 治理：工具执行期间当前 Agent 的身份上下文（git_* 工具自动读取 trailers）──
# 在 core/llm.py / core/executor.py 发起工具调用前 set；若缺失则 git_commit 拒绝提交。
# None = 直调场景（人类用 CLI 打 /git 命令），允许缺省但 trailers 置 "-" + WARNING。
