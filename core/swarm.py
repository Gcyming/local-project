...
class SwarmOrchestrator:
    ...
    @staticmethod
    def _load_requests_concurrency() -> int:
        """读取 config/requests.json 的 concurrency（1-20 合法）；缺失/损坏回退默认 4"""
        try:
            here = os.path.dirname(os.path.abspath(__file__))
            cfg_path = os.path.join(here, "..", "config", "requests.json")
            with open(cfg_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            c = int(data.get("concurrency", 4))
            if 1 <= c <= 20:
                return c
        except Exception:
            pass
