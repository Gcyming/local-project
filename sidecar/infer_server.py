...
# ============================================================
# SILAM-Σ 集成（4C 阶段：非破坏性新增，默认关闭）
# ============================================================

_silam_bridge = None
_silam_agent_id = os.environ.get("SILAM_AGENT_ID", "default")
_silam_backbone = os.environ.get("SILAM_BACKBONE_PATH")


def _init_silam_bridge() -> None:
    """延迟初始化 SILAM 桥接器（首次请求时加载，避免启动阻塞）。"""
    global _silam_bridge
    if _silam_bridge is not None:
        return
    try:
        from sidecar.silam_bridge import init_bridge, SilamConfig
        cfg = SilamConfig()
        # 检查 slime.toml [silam] enabled
        toml_path = _PROJECT_ROOT / "slime.toml"
        if toml_path.exists():
            try:
                with toml_path.open("rb") as f:
                    _toml = tomllib.load(f)
                _silam_cfg = _toml.get("silam", {})
                if not _silam_cfg.get("enabled", False):
                    logger.info("[silam] SILAM 未在 slime.toml 中启用，跳过")
                    return
                logger.info(f"[silam] SILAM 已启用，agent_id={_silam_agent_id}")
            except Exception as e:
                logger.warning(f"[silam] 读取 slime.toml 失败: {e}")
        _silam_bridge = init_bridge(_silam_agent_id, cfg, _silam_backbone)
        logger.info(f"[silam] 桥接器初始化完成")
    except ImportError as e:
        logger.warning(f"[silam] 导入失败，SILAM 功能不可用: {e}")
    except Exception as e:
        logger.warning(f"[silam] 初始化异常: {e}")


@app.get("/silam/health")
async def silam_health():
    """SILAM 健康检查端点。"""
    if _silam_bridge is None:
        return {"status": "disabled", "service": "silam"}
    try:
        status = _silam_bridge.status()
        return {"status": "ok", "service": "silam", **status}
    except Exception as e:
        return {"status": "error", "service": "silam", "error": str(e)}


@app.post("/silam/inference")
async def silam_inference(request: Request):
    """SILAM 推理端点（§8.1 请求 / §8.2 响应）。"""
    body = await request.json()
    rid = str(body.get("request_id", ""))
    t = str(body.get("type", "")).strip().lower()
    payload = body.get("payload", {})

    if t != "inference":
        return JSONResponse(status_code=400, content={
            "request_id": rid, "status": "error",
            "error": f"未知 type: {t}，SILAM 只支持 inference"})
    ...
    if _silam_bridge is None:
        return JSONResponse(status_code=503, content={
            "request_id": rid, "status": "error",
            "error": "SILAM 未启用或未初始化"})

    try:
        result = _silam_bridge.inference(payload)
        return {"request_id": rid, "status": "success", "payload": result}
    except Exception as e:
        import traceback
        logger.error(f"[silam] inference 失败: {e}\n{traceback.format_exc()}")
...
@app.post("/silam/save")
async def silam_save(request: Request):
    body = await request.json()
    rid = str(body.get("request_id", ""))
    if _silam_bridge is None:
        _init_silam_bridge()
...
@app.get("/silam/status")
async def silam_status():
    if _silam_bridge is None:
        _init_silam_bridge()
    if _silam_bridge is None:
        return {"status": "disabled", "service": "silam"}
    try:
        return {"status": "ok", "service": "silam", **_silam_bridge.status()}
    except Exception as e:
        return {"status": "error", "service": "silam", "error": str(e)}
